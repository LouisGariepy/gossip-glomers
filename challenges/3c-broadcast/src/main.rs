...
type Node = common::node::Node<
    NodeState,
    InboundBroadcastRequest,
    OutboundBroadcastResponse<'static>,
    OutboundBroadcastRequest<'static>,
    InboundBroadcastResponse,
>;
...
async fn main() {
    ...
    tokio::spawn(spawn_healing(node.clone()));
    node.run(spawn_run);
}
            ...
            tokio::spawn(async move {
                ...
                let req_json = {
                    ...
                    node.serialize(Message {
                        ...
                        dest: neighbour_id.into(),
                        body: Request {
                            msg_id,
                            kind: OutboundBroadcastRequest::BroadcastMany {
                                messages: &messages.as_slice()[timeout_timestamp..],
                            },
                        },
                    })
                };
                ...
                let response = node.rpc_json(msg_id, req_json).await;
...
async fn spawn_run(node: Arc<Node>, msg: Message<Request<InboundBroadcastRequest>>) {
    match msg.body.kind {
            ...
            if inserted {
                ...
                for neighbour in &node.state.neighbours {
                    ...
                    tokio::spawn(async move {
                        ...
                        // Send RPC request
                        let response = node.rpc(rpc_msg).await;
